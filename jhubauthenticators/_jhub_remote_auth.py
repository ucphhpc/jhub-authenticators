...
class DataRemoteUserAuthenticator(RemoteUserAuthenticator):
    ...
    data_headers = List(
        default_value=[], config=True, help="""List of allowed data headers"""
    )
    ...
    def authenticate(self, handler, data):
        ...
        user = {"name": encoded_name, "auth_state": {"real_name": real_name}}
    ...
    def pre_spawn_start(self, user, spawner):
        ...
        if isinstance(auth_state, dict) and "real_name" in auth_state:
            ...
            self.log.debug(
                "Pre-Spawn: {} set user real_name {}".format(user, user.real_name)
            )
